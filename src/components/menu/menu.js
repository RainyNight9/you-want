...
export default {
  ...
  methods: {
    renderIcon: function (h, menu, key) {
      const icon = menu.meta ? menu.meta.icon : 'none'
      if (this.$scopedSlots.icon && icon && icon !== 'none') {
        ...
        return vnodes
      }else if(!icon && menu.meta?.img) {
        return <img class="anticon anticon-appstore" src={menu.meta.img} style="width:17px;height:16px;"/>
      }else{
    ...
    renderMenuItem: function (h, menu) {
      ...
      return h(
        ...
        [
          h(tag, config,
            [
              this.renderIcon(h, menu, menu.fullPath),
