...
除了以上颜色外，可以使用任何的颜色，只需要在 [src/config/config.js](../config/config.js) 文件中配置的主题色即可。如：

```js
module.exports = {
  theme: {
    color: '#2885FF', // 换成任何的颜色，支持 hex 色值
    mode: 'dark',  // 设置默认主题模式，可选 light、dark 和 night
  ...
  showLang: false,
  fixedHeader: true,
  layout: 'side', // //设置默认导航布局，有 side 和 head 可选
  systemName: 'you-want',
  weekMode: false,   //有视觉障碍的群体,色弱模式，true:开启，false:不开启
}
```

当设置好主题色后，系统会根据这个主题色生成一系列配套的颜色，并应用到vue组件中。

:::tip
可以在样式文件中直接使用 less 变量 ``@theme-color``。
:::
...
也可以在 [src/config/config.js](../config/config.js) 重新定义这些功能色

:::tip
想在样式文件中使用以上各功能色，引用各功能色对应的 less 变量即可。
:::
...
:::tip
想在样式文件中使用以上文本色，引用各文本色对应的 less 变量即可。
:::

:::warning
目前不支持自定义文本色，因为涉及到主题模式切换时文本色的置换问题。如强行修改，可能会导致主题模式切换时出现样式异常。
如果项目不需要主题模式切换，可自行替换以上文本色。
:::
...
:::tip
想在样式文件中使用以上背景色，引用各背景色对应的 less 变量即可。
:::
:::warning
目前也不支持自定义背景色，因为涉及到主题模式切换时背景色的置换问题。如强行修改，可能会导致主题模式切换时出现样式异常。
如果项目不需要主题模式切换，可自行替换以上背景色。
:::

### antd 的色系
除了以上颜色，还引入了 ant-design 内置的色系。如下：
...
以上色系对应的less变量均可以在样式代码中直接使用。

:::tip
建议在开发中使用 `less变量` 而不是直接使用 `颜色值` 来设置颜色。这样做对主题色和主题模式切换很有帮助。
:::

## 主题模式

有三种主题模式，分别为：`light/亮色菜单模式`、`dark/暗色菜单模式` 和 `night/黑夜模式`。

可以在这三种模式之间随意切换，也可以在 [src/config/config.js](../config/config.js) 中设置默认的主题模式。

## 导航布局

有两种导航布局，`side/侧边导航` 和 `head/顶部导航`。  
默认为侧边导航，可以在 [src/config/config.js](../config/config.js) 中修改导航布局

## 动画

内置了 [animate.css](https://animate.style) 动画库，在页面切换时会应用动画效果。可以在 [src/config/config.js](../config/config.js) 中配置动画效果或者禁用动画。

支持的动画特效种类，可以参考 src/config/default/animate.config.js 文件。

:::tip
以上所有主题设置项，均已映射到 vuex/setting 模块的 state 中，可以通过提交 setting/mutations 实时修改设置项。  
